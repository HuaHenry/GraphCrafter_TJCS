...
def show_enhancement(src,type,size):
    width = size[0]
    height = size[1]
    if type == 'White Balance 1':
        balance_img1 = white_balance(src)
        plt_save(balance_img1, 'White Balance 1',width,height)

    # 灰度世界算法
    if type == 'White Balance 2':
        balance_img2 = grey_world(src)
        plt_save(balance_img2, 'White Balance 2',width,height)

    # 直方图均衡化
    if type == 'White Balance 3':
        balance_img3 = his_equl_color(src)
        plt_save(balance_img3, 'White Balance 3',width,height)

    # 视网膜-大脑皮层(Retinex)增强算法
    # 单尺度Retinex
    if type == 'Single Scale Retinex 1':
        ssr1 = single_scale_retinex(src, 300)
        ssr1[ssr1 < 0] = 0
        plt_save(ssr1, 'Single Scale Retinex 1',width,height)

    if type == 'Single Scale Retinex 2':
        ssr2 = s_s_r(src)
        plt_save(ssr2, 'Single Scale Retinex 2',width,height)

    # 多尺度Retinex
    if type == 'Multi Scale Retinex 1':
        msr1 = multi_scale_retinex(src, [15, 80, 250])
        msr1[msr1 < 0] = 0
        plt_save(msr1, 'Multi Scale Retinex 1',width,height)

    if type == 'Multi Scale Retinex 2':
        msr2 = m_s_r(src, sigma_list=[15, 80, 250])
        plt_save(msr2, 'Multi Scale Retinex 2',width,height)

    if type == 'Multi Scale Retinex With Color Restoration 1':
        msrcr1 = m_s_r_c_r(src, sigma_list=[15, 80, 250])
        plt_save(msrcr1, 'Multi Scale Retinex With Color Restoration 1',width,height)

    # 自动白平衡 AWB
    if type == 'Automatic White Balance':
        awb = automatic_white_balance(src)
        plt_save(awb, 'Automatic White Balance',width,height)

    # 自动色彩均衡 ACE
    if type == 'Automatic Color Equalization':
        balance_img4 = automatic_color_equalization(src)
        plt_save(balance_img4, 'Automatic Color Equalization',width,height)
